...
export async function POST(req) {
  try {
    ...
    // Forward file to Sightengine using browser-style FormData
    const forwardForm = new FormData();
    forwardForm.append("media", file); // ⚠️ Key must be 'media'
    ...
    const response = await fetch(`https://api.sightengine.com/1.0/check.json?${params}`, {
      method: "POST",
      body: forwardForm, // ✅ browser FormData
    });
